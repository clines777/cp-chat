 ...
 * 用户解禁
 */
class UnbanHandler implements MessageHandlerInterface
{

    public function getMsgType(): string
    {
        return MsgType::UnbanUser;
    }
    ...
    public function handle(Server $server, int $fd, MsgPayload $payload): void
    {
        try {
            ...
            if ((int)$guUser->role_type !== GroupUser::RoleUser) {
                Helper::push($server, $fd, MsgPayload::error(ErrorCode::ErrInvalidOperate, $payload, '解禁对象不是一般用户'));

                return;
            }

            $unbanParam = ['is_ban' => GroupUser::BanNo];
            $ok         = true;
            if ((int)$guUser->is_ban !== GroupUser::BanNo) {
                $ok = $groupUserRepo->updateGroupUser($guUser->group_id, $guUser->user_id, $unbanParam);
            }
            if ($ok) {
                $onlineUser = $groupUserRepo->rGetGroupOnlineUser($guUser->group_id, $guUser->user_id);
                if ( ! empty($onlineUser)) {
                    $ok = $groupUserRepo->rSetGroupOnlineUser($guUser->group_id, $guUser->user_id, $unbanParam);
                }
            }
            if ( ! $ok) {
                Helper::push($server, $fd, MsgPayload::error(ErrorCode::ErrUnBanFailed, $payload, '解禁失败'));
            ...
            $userLogger->setParam($user->id, $user->ext_member_id, $user->ext_username)->sceneInApp()->setLogType(UserLogBuilder::TypeGroupUnban)->byGroupAdmin(
                $admin->id,
                $admin->ext_member_id,
                $admin->ext_username,
            )->withRemark(sprintf('用户解禁，群号: %s', $group['code']));

            /** @var UserLogRepository $userLogRepo */
            $userLogRepo = Container::get(UserLogRepository::class);
            $userLogRepo->addWithBuilder($userLogger);

            Helper::push($server, $fd, MsgPayload::make(MsgType::UnBanUserOK, ['target_user_id' => $guUser->user_id], $payload->getMeta()));
            RStream::push(RStream::TypeUnbanUser, ['group_id' => $guUser->group_id, 'user_id' => $guUser->user_id]);
        } catch (\Throwable $e) {
            Log::error(sprintf('用户解禁异常! payload: %s, 讯息: %s', $payload->jsonSerialize(), Helper::getExpDetails($e)));
            Helper::push($server, $fd, MsgPayload::error(ErrorCode::ErrUnBanFailed));
        }
    }
